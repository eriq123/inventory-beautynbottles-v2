        ...
        <v-card flat>
          <v-card-title>
            ...
            <v-dialog v-model="dialog.show" max-width="500px">
              <v-card>
                <v-card-title>
                  <span class="headline">{{ this.dialog.action }}</span>
                </v-card-title>
                <v-card-text>
                  <v-row>
                    <v-col sm="12">
                  ...
                  <v-row>
                    <v-col cols="6">
                      <v-text-field
                        type="number"
                        label="Quantity"
                        v-on:keyup.enter="processAddAndUpdate"
                        v-model="dialog.quantity"
                        autofocus
                      >
                        <template #append-outer>
                          <app-raw-menu
                            :menu="dialog.convert_collection"
                            :selected="dialog.convert_name"
  ...
  methods: {
    ...
    async attach() {
      ...
      if (!this.duplicateAttach) {
        this.dialog.quantity = this.dialog.quantity * this.dialog.convert_value;
        this.assembled.loading = true;
        await axios
