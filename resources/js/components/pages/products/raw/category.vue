      ...
      <v-card flat>
        ...
        >
          ...
          <template #item.actions="{ item }">
            <v-btn text color="blue darken-4" @click.stop="showUpdateForm(item)"
              >Update</v-btn
            >
            <v-btn text color="red darken-4" @click.stop="deleteProcess(item)"
              >Delete</v-btn
            >
          </template>
    ...
    processUpdate() {
      axios
        ...
        .then(response => {
          ...
          Object.assign(this.items[this.itemIndex], response.data.category);
          this.loading = false;
        })
        .catch(error => {
          if (error.response) {
